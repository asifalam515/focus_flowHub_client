...
const extractErrorMessage = (payload: unknown, fallback: string) => {
  ...
  if (errorObj && typeof errorObj === "object") {
    const nested = (errorObj as Record<string, unknown>).message;
...
const normalizeUser = (
  ...
  return {
    id: typeof u.id === "string" && u.id.trim() ? u.id : `user-${Date.now()}`,
    name: typeof u.name === "string" && u.name.trim() ? u.name : fallback.name,
    email:
      typeof u.email === "string" && u.email.trim() ? u.email : fallback.email,
    avatar: typeof u.avatar === "string" ? u.avatar : "",
    points: typeof u.points === "number" ? u.points : 0,
    achievements: Array.isArray(u.achievements)
      ? u.achievements.filter((x): x is string => typeof x === "string")
      : [],
  };
};
...
const authRequest = async (
  ...
  fallbackUser: { name: string; email: string },
) => {
  const response = await fetch(`${AUTH_BASE_URL}/auth/${endpoint}`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    throw new Error(extractErrorMessage(payload, `Failed to ${endpoint}`));
  }
  ...
  const token =
    ...
    "";
  const userRaw = data.user ?? source.user ?? data;

  return {
    user: normalizeUser(userRaw, fallbackUser),
    token,
  };
};
...
export const dummyNotifications: Notification[] = [
  ...
  {
    id: "n2",
    message: "Your reading streak is at 7 days!",
    read: false,
    createdAt: new Date().toISOString(),
  },
  {
    id: "n3",
    message: "New goal milestone reached!",
    read: true,
    createdAt: new Date().toISOString(),
  },
  {
    id: "n4",
    message: "⏰ 3 tasks are overdue — review your inbox!",
    read: false,
    createdAt: new Date().toISOString(),
  },
  {
    id: "n5",
    message: "📋 Weekly review reminder: summarize your week",
    read: false,
    createdAt: new Date().toISOString(),
  },
];
...
export const dummyProjects: Project[] = [
  ...
  },
  {
    ...
    description: "12-week training plan with diet tracking",
    color: "hsl(36 90% 55%)",
  ...
  },
  {
    ...
    description: "Plan and execute content for Q1",
    color: "hsl(280 60% 55%)",
...
export const dummyTasks: Task[] = [
  ...
  {
    id: "t-i1",
    title: "Look into new CI/CD tools",
    description: "",
    dueDate: "",
    priority: "low",
    completed: false,
  ...
  {
    id: "t-i3",
    title: "Research meal prep services",
    description: "",
    dueDate: "",
    priority: "low",
    completed: false,
  ...
  {
    id: "t-i4",
    title: "Check out Rust programming language",
    description: "",
    dueDate: "",
    priority: "low",
    completed: false,
  ...
  // Next actions (clarified & actionable)
  {
    ...
    description: "Create 3 variations for the new product landing page",
    dueDate: "2026-03-09",
    startTime: "09:00",
    endTime: "11:00",
    priority: "high",
  ...
  },
  {
    ...
    description: "Draft and schedule this week's newsletter",
    dueDate: "2026-03-10",
    startTime: "14:00",
    endTime: "15:30",
    priority: "medium",
  ...
  },
  {
    ...
    description: "Gather feedback and create presentation",
    dueDate: "2026-03-12",
    startTime: "15:00",
    endTime: "16:00",
    priority: "low",
  ...
  },
  {
    ...
    description: "Finish exercises in chapter 8",
    dueDate: "2026-03-09",
    startTime: "13:00",
    endTime: "14:30",
    priority: "medium",
  ...
  },
  {
    ...
    description: "Week 4, Day 2 of training plan",
    dueDate: "2026-03-08",
    startTime: "07:00",
    endTime: "08:00",
    priority: "high",
  ...
  // Completed
  {
    ...
    description: "Review 4 pending PRs from the team",
    dueDate: "2026-03-08",
    priority: "medium",
    completed: true,
    status: "completed",
  ...
  },
  {
    ...
    description: "Sidebar doesn't collapse on mobile",
    dueDate: "2026-03-08",
    priority: "high",
    completed: true,
    status: "completed",
  ...
  },
  {
    ...
    description: "Initialize project with proper tsconfig",
    dueDate: "2026-03-05",
    priority: "medium",
    completed: true,
    status: "completed",
...
export const dummyGoals: Goal[] = [
  ...
  {
    id: "g3",
    title: "Read 4 books this quarter",
    type: "reading",
    target: 4,
    current: 2,
    deadline: "2026-03-31",
  ...
  {
    id: "g4",
    title: "Exercise 20 times this month",
    type: "habit",
    target: 20,
    current: 15,
    deadline: "2026-03-31",
...
export const dummyBooks: Book[] = [
  {
    ...
    totalPages: 320,
    completedPages: 320,
    notes: "Great insights on building systems over goals.",
    summary:
      "A practical guide to building good habits and breaking bad ones using the four laws of behavior change.",
    createdAt: "2026-01-10T00:00:00Z",
  },
  {
    id: "b2",
    ...
    totalPages: 296,
    completedPages: 180,
    notes: "The concept of attention residue is fascinating.",
  ...
  },
  {
    ...
    totalPages: 352,
    completedPages: 90,
    notes: "DRY and orthogonality concepts are key.",
  ...
  },
  {
    ...
    completedPages: 45,
    notes: "",
    summary: "Explores the two systems of thinking that drive our decisions.",
    createdAt: "2026-03-01T00:00:00Z",
  },
];
...
export const api = {
  login: async (email: string, password: string) => {
    return authRequest("login", { email, password }, { name: "User", email });
  },
  signup: async (email: string, password: string, name: string) => {
    return authRequest("signup", { name, email, password }, { name, email });
  },
  ...
  updateTask: async (id: string, data: Partial<Task>): Promise<Task> => {
    await delay();
    const t = dummyTasks.find((t) => t.id === id);
    return { ...t!, ...data };
  },
  ...
  getInbox: async (): Promise<Task[]> => {
    await delay();
    return dummyTasks.filter((t) => t.status === "inbox");
  },
  ...
  updateProject: async (
    ...
    await delay();
    const p = dummyProjects.find((p) => p.id === id);
    return { ...p!, ...data };
  },
  ...
  updateHabit: async (id: string, data: Partial<Habit>): Promise<Habit> => {
    await delay();
    const h = dummyHabits.find((h) => h.id === id);
    return { ...h!, ...data };
  },
  deleteHabit: async (_id: string): Promise<void> => {
    await delay();
  },
  ...
  updateGoal: async (id: string, data: Partial<Goal>): Promise<Goal> => {
    await delay();
    const g = dummyGoals.find((g) => g.id === id);
    return { ...g!, ...data };
  },
  ...
  updateBook: async (id: string, data: Partial<Book>): Promise<Book> => {
    await delay();
    const b = dummyBooks.find((b) => b.id === id);
    return { ...b!, ...data };
  },
  deleteBook: async (_id: string): Promise<void> => {
    await delay();
  },
